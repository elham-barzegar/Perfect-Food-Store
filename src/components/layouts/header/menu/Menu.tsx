                    ...
                    <div id="all_cat_inner_box" className="flex flex-wrap justify-between gap-y-[15px]">
...

                        {
                            ...
                            categoryMenuItems.data.map((item: EntityType<MenuItemType>, index:number) => {
                                return <IconBox key={index} link={item.attributes.link} icon={item.attributes.icon_name} size={30}
                                                title={item.attributes.title} titleClassName={"text-heading-sm text-blue-300"}
